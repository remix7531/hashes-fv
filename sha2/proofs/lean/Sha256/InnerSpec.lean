...
Both `Sha256.lean::sha256_impl_spec` (the SHA-256 corollary) and
...
The SHA-256 corollary (`Sha256.lean::sha256_impl_spec`) instantiates this
...
theorem sha2_inner_spec
  ...
  have hbl : out.val * 64 ≤ data.length := by
    rw [hblocks]; exact Nat.div_mul_le_self data.length 64
  have hloop0 :
  ...
  have hsum : out.val * 64 + remaining.val = data.length := by
    rw [hrem_mod, hblocks]; have := Nat.div_add_mod data.length 64; omega
  ...
  have hfull_match : ∀ (i : Nat) (hi : i < 64),
    ...
    · rw [List.getElem!_set_ne _ remaining.val i 128#u8 (by unfold Nat.not_eq; omega)]
  ...
  · /- `≥ 56` branch — extra compression block carries the length tag. -/
    ...
    have hlow_zero : ∀ (i : Nat) (hi : i < 56),
        toUInt8 ((Array.repeat 64#usize (0#u8 : U8)).val[i]'(by simp; omega)) =
          vB[i]'(by omega) := by
      intro i hi
      have hbyte : (Array.repeat 64#usize (0#u8 : U8)).val[i]'(by simp; omega) =
          (0#u8 : U8) := by
  ...
  · /- `< 56` branch — single final block carries marker + length tag. -/
    ...
    rw [hstate2, hstate_prefix, hfb_full]; simp only [hsz, if_pos hcond]; rfl
