...
theorem loop0_action_spec
  ...
  have hi64 : i.val * 64 + 64 ≤ data.length := by scalar_tac +nonLin
  step as ⟨i1, hi1⟩
  ...
  simp only [show s1.len = 64#usize from by apply UScalar.eq_of_val_eq; exact hs1_len,
    ↓reduceDIte, core.result.Result.unwrap, lift, Array.to_slice, bind_tc_ok]
  ...
  have hu32 : Impl.toU32s (arrayU8ToVec block) =
    ...
    have hbck : arrayU8ToVec block =
      ...
      have hs1_take : s1.val = (s.val).take 64 := by rw [hs1_val]; simp [List.slice]
      rw [List.getElem_of_eq hs1_take, List.getElem_take, List.getElem_of_eq hs_val,
          List.getElem_drop]
      simp [hi1]; rfl
    rw [hbck]
  rw [hu32]
...
theorem sha256_inner_loop0_spec
  ...
  have haction_view : ∀ (i : Usize) (s : Array U32 8#usize) (h : i.val < blocks.val),
      action i s ⦃ s' => arrayU32ToVec s' = loop0_step data blocks
        (arrayU32ToVec s) ⟨i.val, h⟩ ⦄ := fun i s h => by
    simp only [haction_def, loop0_step]; exact loop0_action_spec data blocks s hbl i h
  classical
  set f' : Array U32 8#usize → Fin blocks.val → Array U32 8#usize :=
    fun s i =>
      match action ⟨i.val, by scalar_tac⟩ s with
      | .ok v => v
      | .fail _ => s
      | .div => s with hf'_def
  have hf'_spec : ∀ (i : Usize) (s : Array U32 8#usize) (h : i.val < blocks.val),
      action i s ⦃ s' => s' = f' s ⟨i.val, h⟩ ⦄ := fun i s h => by
    obtain ⟨v, hact, _⟩ := ok_of_spec (haction_view i s h)
    simp only [hf'_def,
      show (⟨i.val, by scalar_tac⟩ : Usize) = i from UScalar.eq_of_val_eq rfl, hact, spec_ok]
  have hf'_to_view : ∀ (i : Fin blocks.val) (s : Array U32 8#usize),
      arrayU32ToVec (f' s i) = loop0_step data blocks (arrayU32ToVec s) i := fun i s => by
    obtain ⟨v, hact, hpost⟩ := ok_of_spec (haction_view
      ⟨i.val, by scalar_tac⟩ s i.isLt)
  ...
  have hfold_conv :
    ...
    intro n
    induction n with
    | zero => intro _ _; simp [Fin.foldl_zero]
    | succ k ih =>
      intro hn s
      rw [Fin.foldl_succ_last, Fin.foldl_succ_last]
      simp only [Fin.val_last]
      rw [hf'_to_view ⟨k, hn⟩ _]
      exact congrArg (loop0_step data blocks · ⟨k, hn⟩) (ih (Nat.le_of_succ_le hn) s)
