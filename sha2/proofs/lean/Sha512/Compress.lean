...
theorem compress_u64_loop1_spec
  ...
  set init : UInt64 × UInt64 × UInt64 × UInt64 ×
             UInt64 × UInt64 × UInt64 × UInt64 :=
    (toUInt64 a, toUInt64 b, toUInt64 c, toUInt64 d,
     toUInt64 e, toUInt64 f, toUInt64 g, toUInt64 h) with hinit
  ...
  · rintro ⟨iter, a', b', c', d', e', f', g', h'⟩ ⟨hend, hle, hcur⟩
    ...
    · -- Step case
      have hbody := compress_u64_loop1_body_spec w iter a' b' c' d' e' f' g' h' hlt hend
      simp only [spec, theta] at hbody ⊢
      revert hbody
      cases hev : Extraction.sha512.soft_compact.compress_u64_loop1.body w iter
                    a' b' c' d' e' f' g' h' with
      | ok r =>
        intro hbody
        simp only [wp_return] at hbody
        obtain ⟨start', aN, bN, cN, dN, eN, fN, gN, hN,
        ...
        · show (toUInt64 aN, toUInt64 bN, toUInt64 cN, toUInt64 dN,
          ...
          rw [hstart_val,
              partialFinFoldl_succ 80
                ...
                iter.start.val hlt,
              ← hcur]
          exact Prod.ext ha_eq (Prod.ext hb_eq (Prod.ext hc_eq
            (Prod.ext hd_eq (Prod.ext he_eq (Prod.ext hf_eq (Prod.ext hg_eq hh_eq))))))
        · show 80 - start'.val < 80 - iter.start.val; omega
      | fail e => intro h; exact h.elim
      | div => intro h; exact h.elim
    · -- Done case
      have hstart_eq : iter.start.val = 80 := by omega
      ...
      simp only [hnlt, ↓reduceIte, bind_tc_ok]
      simp only [spec, theta]
      have hfull : partialFinFoldl 80
                     (fun s (i : Fin 80) => tupledRoundStep64 (arrayU64ToVec w) i.val s)
                     iter.start.val init =
                   Fin.foldl 80
                     (fun s (i : Fin 80) => tupledRoundStep64 (arrayU64ToVec w) i.val s) init := by
...
theorem compress_u64_loop0_spec
    ...
    ⦃ w' =>
      arrayU64ToVec w' =
        Fin.foldl 64
          ...
          (arrayU64ToVec w) ⦄ := by
  set init : SHS.SHA512.Impl.Schedule := arrayU64ToVec w with hinit
  ...
  · rintro ⟨iter, w'⟩ ⟨hend, hlo, hle, hcur⟩
    simp only at hend hlo hle hcur
    by_cases hlt : iter.start.val < 80
    · -- Step case
      have hbody := compress_u64_loop0_body_spec iter w' hlo hlt hend
      simp only [spec, theta] at hbody ⊢
      revert hbody
      cases hev : Extraction.sha512.soft_compact.compress_u64_loop0.body iter w' with
      | ok r =>
        intro hbody
        simp only [wp_return] at hbody
        obtain ⟨start', w'', hstart_val, hr_eq, hext⟩ := hbody
        subst hr_eq
        simp only [wp_return]
        refine ⟨⟨?_, ?_, ?_, ?_⟩, ?_⟩
        · exact hend
        · show 16 ≤ start'.val; omega
        · show start'.val ≤ 80; omega
        · -- Partial fold extends by one step.
          show arrayU64ToVec w'' =
                 partialFinFoldl 64
                   (fun sched (i : Fin 64) => extScheduleStep (16 + i.val) sched)
                   (start'.val - 16) init
          have hidx : start'.val - 16 = (iter.start.val - 16) + 1 := by omega
          have hbnd : iter.start.val - 16 < 64 := by omega
          rw [hidx,
              partialFinFoldl_succ 64
                (fun sched (i : Fin 64) => extScheduleStep (16 + i.val) sched)
                (iter.start.val - 16) hbnd,
              ← hcur]
          have hshift : 16 + (iter.start.val - 16) = iter.start.val := by omega
          rw [show (⟨iter.start.val - 16, hbnd⟩ : Fin 64).val = iter.start.val - 16
                from rfl, hshift]
          exact hext
        · show 80 - start'.val < 80 - iter.start.val; omega
      | fail e => intro h; exact h.elim
    ...
    · -- Done case: iter.start = 80
      ...
      have hfull : partialFinFoldl 64
                     (fun sched (i : Fin 64) => extScheduleStep (16 + i.val) sched)
                     (iter.start.val - 16) init =
                   Fin.foldl 64
                     (fun sched (i : Fin 64) => extScheduleStep (16 + i.val) sched) init := by
        rw [show iter.start.val - 16 = 64 from by omega, partialFinFoldl_full]
      rw [hfull] at hcur
      exact hcur
  · -- Initial invariant
    refine ⟨rfl, ?_, ?_, ?_⟩
...
/-- Bridge `Vector.set!` to bounded `Vector.set` when the index is in bounds. -/
private theorem Vector.set!_eq_set_of_lt
    {α : Type _} {n : Nat} (v : Vector α n) (i : Nat) (x : α) (h : i < n) :
    v.set! i x = v.set i x h := by
  apply Vector.ext
  intro k hk
  show (v.toArray.setIfInBounds i x)[k]'(by simp; omega) = _
  rw [Array.getElem_setIfInBounds, Vector.getElem_set]
  split <;> rfl
...
private theorem arrayU64ToVec_schedule_copy
  ...
  have hFfold_get : ∀ (n : Nat) (_hn : n ≤ 16),
    ...
    induction n with
    | zero =>
      simp only [partialFinFoldl_zero, Nat.not_lt_zero, ↓reduceIte]
      simp
    | succ j ih =>
      have hj : j < 16 := hn
      have hj' : j ≤ 16 := Nat.le_of_lt hj
      rw [partialFinFoldl_succ 16 _ j hj]
      have ih' := ih hj'
      rw [Vector.getElem_set (hi := Nat.lt_of_lt_of_le hj (by decide))]
      by_cases heq : j = k
      · subst heq
        simp [show j < j + 1 from Nat.lt_succ_self _]
      · simp only [heq, ↓reduceIte]
        rw [ih']
        by_cases hklt : k < j
        · simp [hklt, show k < j + 1 from Nat.lt_succ_of_lt hklt]
        · simp [hklt, show ¬ k < j + 1 from by omega]
  ...
  /- RHS rewrite via hfold_at_k.  The goal's RHS bound is `k < ↑80#usize`
     while hfold_at_k uses `k < 80`.  Use `trans` to bridge via the explicit
     normal form. -/
  trans (if k < 16 then (arrayU64ToVec block).toList[k]! else (0 : UInt64))
  swap
  · exact hfold_at_k.symm
  have h80val : ((80#usize : Usize).val : Nat) = 80 := rfl
  /- Massage the LHS: `(↑w1)[k]` has bound that depends on `↑w1.length`;
     bridge to `w1.val[k]'(< 80)` form to make `rw [hw1_val]` work. -/
  show toUInt64 (w1.val[k]'(by rw [hw1_len]; exact hk80)) =
  ...
  · /- k < 16: w1.val[k] = block.val[k]. -/
    ...
    have hk_in_set : k < ((Array.repeat 80#usize (0#u64 : U64)).val.setSlice! 0 block.val).length := by
      rw [List.length_setSlice!, Array.repeat_val, List.length_replicate]; omega
    rw [show w1.val[k]'hk_in_w1 = w1.val[k]! from by
      rw [List.getElem!_eq_getElem?_getD, List.getElem?_eq_getElem hk_in_w1]; rfl]
    rw [hw1_val]
    rw [List.getElem!_setSlice!_middle _ _ 0 k (by
      refine ⟨by omega, ?_, ?_⟩
      · rw [hblen]; omega
      · rw [Array.repeat_val, List.length_replicate]; omega)]
    have hk_in_block : k < block.val.length := by rw [hblen]; exact hk16
    rw [show block.val[k - 0]! = block.val[k]'hk_in_block from by
      rw [show k - 0 = k from rfl, List.getElem!_eq_getElem?_getD,
        List.getElem?_eq_getElem hk_in_block]; rfl]
    rw [arrayU64ToVec_toList]
    simp [hblen, hk16]
  · /- k ≥ 16: w1.val[k] = 0. -/
    simp only [hk16, ↓reduceIte]
    have hk16 : 16 ≤ k := Nat.not_lt.mp hk16
    have hk_in_w1 : k < w1.val.length := by rw [hw1_len]; exact hk80
    rw [show w1.val[k]'hk_in_w1 = w1.val[k]! from by
      rw [List.getElem!_eq_getElem?_getD, List.getElem?_eq_getElem hk_in_w1]; rfl]
    rw [hw1_val]
    rw [List.getElem!_setSlice!_suffix _ _ 0 k (by rw [hblen]; omega)]
    rw [Array.repeat_val]
    have hk_rep_lt : k < (List.replicate 80#usize.val (0#u64 : U64)).length := by
      rw [List.length_replicate]; exact hk80
    rw [show (List.replicate 80#usize.val (0#u64 : U64))[k]! =
          (List.replicate 80#usize.val (0#u64 : U64))[k]'hk_rep_lt from by
      rw [List.getElem!_eq_getElem?_getD, List.getElem?_eq_getElem hk_rep_lt]; rfl]
...
theorem compress_u64_spec
  ...
  have hs2_val : s2.val = block.val := by
    rw [← hs2_eq, hs1_val]; simp [Array.val_to_slice]
  have hw1_val : w1.val =
      (Array.repeat 80#usize (0#u64 : U64)).val.setSlice! 0 block.val := by
    rw [hw1_def, hd1.2.2 s2, hs2_val]
  ...
  rintro ⟨a1, b1, c1, d1, e1, f1, g1, h1⟩ hloop1
  simp only at hloop1
  obtain ⟨ha1, hb1, hc1, hd1', he1, hf1, hg1, hh1⟩ := hloop1
  ...
  have hsched_eq :
    ...
    let extStep : Nat → SHS.SHA512.Impl.Schedule → SHS.SHA512.Impl.Schedule :=
      fun n w => extScheduleStep n w
    have hfStep_split :
        ∀ (w : SHS.SHA512.Impl.Schedule) (t : Fin 80),
          fStep w t =
            (if t.val < 16 then copyStep t.val w else extStep t.val w) := by
      intro w t
      simp only [fStep, SHS.Equiv.SHA512.Compress.Impl.implScheduleStep,
                 extScheduleStep, copyStep, extStep]
      ...
      · simp only [ht, ↓reduceIte, ↓reduceDIte]
        ...
        rw [Vector.set!_eq_set_of_lt _ _ _ h80]
        congr 1
        rw [arrayU64ToVec_toList]
        simp [block.property, ht]
      · simp only [ht, ↓reduceIte]
    rw [show Fin.foldl 80 fStep fInit =
          Fin.foldl 80
            (fun w (t : Fin 80) =>
              if t.val < 16 then copyStep t.val w else extStep t.val w) fInit from by
      congr 1; funext w t; exact hfStep_split w t]
    rw [fin_foldl_split 16 80 (by decide) copyStep extStep fInit]
    ...
    have houter_eq :
        Fin.foldl (80 - 16) (fun s (i : Fin (80 - 16)) => extStep (i.val + 16) s)
          (arrayU64ToVec w1) =
        Fin.foldl 64
          (fun sched (i : Fin 64) => extScheduleStep (16 + i.val) sched)
          (arrayU64ToVec w1) := by
  ...
  set init_st : SHS.SHA512.Impl.State :=
    #v[toUInt64 a, toUInt64 b, toUInt64 c, toUInt64 d,
       toUInt64 e, toUInt64 f, toUInt64 g, toUInt64 h] with hinit_st_def
  set init_tup : UInt64 × UInt64 × UInt64 × UInt64 ×
                 UInt64 × UInt64 × UInt64 × UInt64 :=
    (toUInt64 a, toUInt64 b, toUInt64 c, toUInt64 d,
     toUInt64 e, toUInt64 f, toUInt64 g, toUInt64 h) with hinit_tup_def
  ...
  have hstate_init : arrayU64ToVec state = init_st := by
    ...
    have hk_state : k < state.val.length := by rw [state.property]; exact hk8
    /- Lift state.val[k] from the unsafe form to the bounded form to align with
       `a_post : a = (↑state)[0]!` etc.  Then rewrite via arrayU64ToVec_getElem. -/
    have hbang : ∀ (j : Nat) (hj : j < 8),
        toUInt64 (state.val[j]'(by rw [state.property]; exact hj)) = toUInt64 (state.val[j]!) := by
      intro j hj
      rw [getElem!_pos _ _ (by rw [state.property]; exact hj)]
    rw [arrayU64ToVec_getElem state k hk8]
    rw [hbang k hk8]
    /- For each k, the k-th post hypothesis gives a/b/c/d/e/f/g/h_post. -/
    interval_cases k
    · show toUInt64 (state.val[0]!) = toUInt64 a; rw [← a_post]
    · show toUInt64 (state.val[1]!) = toUInt64 b; rw [← b_post]
    · show toUInt64 (state.val[2]!) = toUInt64 c; rw [← c_post]
    · show toUInt64 (state.val[3]!) = toUInt64 d; rw [← d_post]
    · show toUInt64 (state.val[4]!) = toUInt64 e; rw [← e_post]
    · show toUInt64 (state.val[5]!) = toUInt64 f; rw [← f_post]
    · show toUInt64 (state.val[6]!) = toUInt64 g; rw [← g_post]
    · show toUInt64 (state.val[7]!) = toUInt64 h; rw [← h_post]
  /- Bridge: project init_st to init_tup (trivially equal by definition). -/
  have hinit_proj :
      (init_st[0], init_st[1], init_st[2], init_st[3],
       init_st[4], init_st[5], init_st[6], init_st[7]) = init_tup := by
    simp [init_st, init_tup]
  have hbridge := finFoldl_roundStep64_eq_tupled (arrayU64ToVec w2) init_st init_tup hinit_proj
  simp only at hbridge
  ...
  have set_get_ne : ∀ {N : Usize} (a : Aeneas.Std.Array U64 N) (j : Usize) (v : U64) (k : ℕ),
      j.val ≠ k → (a.set j v).val[k]! = a.val[k]! := by
    intro N a j v k hjk
  ...
  rw [out_post, state7_post, state6_post, state5_post, state4_post, state3_post,
      state2_post, state1_post]
  rw [arrayU64ToVec_set8_chain]
  rw [i1_post, i3_post, i5_post, i7_post, i9_post, i11_post, i13_post, i15_post,
      hi, hi2, hi4, hi6, hi8, hi10, hi12, hi14]
  unfold core.num.U64.wrapping_add
  simp only [toUInt64_wrapping_add]
  have hgv : ∀ (k : Nat) (hk : k < 8),
      (arrayU64ToVec state)[k]'(by simp; omega) = toUInt64 (state.val[k]!) := by
    intro k hk
    rw [arrayU64ToVec_getElem state k hk (by simp; omega)]
    rw [getElem!_pos _ _ (by simp [hlen]; omega)]
...
theorem compress512_spec
  ...
  have haction_view :
      ∀ (b : Array U8 128#usize) (s : Array U64 8#usize),
        action b s ⦃ s' => arrayU64ToVec s' = f (arrayU64ToVec s) b ⦄ := by
    intro b s
    simp only [haction_def]
    apply spec_bind (to_u64s_spec b)
    intro words hwords
  ...
  have hf'_spec :
      ∀ (b : Array U8 128#usize) (s : Array U64 8#usize),
        action b s ⦃ s' => s' = f' s b ⦄ := by
    intro b s
    have hview := haction_view b s
    simp only [spec, theta] at hview ⊢
    cases hact : action b s with
    | ok v =>
      rw [hact] at hview
  ...
  have hf'_to_f : ∀ (b : Array U8 128#usize) (s : Array U64 8#usize),
      arrayU64ToVec (f' s b) = f (arrayU64ToVec s) b := by
    intro b s
    have h1 := haction_view b s
    have h2 := hf'_spec b s
  ...
  have hfold_conv : ∀ (l : List (Array U8 128#usize)) (s : Array U64 8#usize),
      arrayU64ToVec (l.foldl f' s) = l.foldl f (arrayU64ToVec s) := by
    intro l
    induction l with
    | nil => intro s; simp
    | cons b bs ih =>
  ...
  have hpost : ∀ (p : Array U64 8#usize),
      p = List.foldl f' state blocks.val →
      arrayU64ToVec p = List.foldl f (arrayU64ToVec state) blocks.val := by
    intro p hp
    rw [hp]; exact hfold_conv _ _
  refine spec_mono ?_ hpost
  convert hloop using 2
  funext p
  congr 1
  funext discr
  obtain ⟨o, iter1⟩ := discr
  cases o <;> rfl
